...
taskForm.addEventListener('submit', function(e) {
    ...
    if (!taskText) { // Якщо текст завдання порожній, показати попередження
...
function renderTasks(date) {
    ...
    if (tasks[date]) { // Якщо є завдання для вибраної дати, відобразити їх
        tasks[date].forEach(function(task) {
            li = document.createElement('li') // Створити елемент списку
            li.textContent = task


            const deleteBtn = document.createElement('button') // Створити кнопку видалення
